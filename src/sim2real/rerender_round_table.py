...
def main():
    ...
    # from src.sim2real.util import convert_obj2usd

    # furniture_assets_root_path = Path(
    #     f"{os.getenv('RARL_SOURCE_DIR')}/sim2real/assets/furniture/mesh/usd"
    # )
    # obj_fn_list = [
    #     "obj/round_table/round_table_base.obj",
    #     "obj/round_table/round_table_leg.obj",
    #     "obj/round_table/round_table_top.obj",
    # ]
    # obj_file_list = [os.path.join(furniture_assets_root_path, fn) for fn in obj_fn_list]
    # convert_obj2usd(obj_file_list[0], overwrite_exists=True, out_type=".usda")
    # convert_obj2usd(obj_file_list[1], overwrite_exists=True, out_type=".usda")
    # convert_obj2usd(obj_file_list[2], overwrite_exists=True, out_type=".usda")
    ...
    pos = config["furniture"]["round_table"]["round_table_top"]["reset_pos"][0]
    ori = config["furniture"]["round_table"]["round_table_top"]["reset_ori"][0][:3, :3]
    view = gen_prim(
        "round_table/round_table_top.usda", "/World/RoundTableTop", pos, ori
    )
    views.append(view)

    pos = config["furniture"]["round_table"]["round_table_leg"]["reset_pos"][0]
    ori = config["furniture"]["round_table"]["round_table_leg"]["reset_ori"][0][:3, :3]
    view = gen_prim(
        "round_table/round_table_leg.usda", "/World/RoundTableLeg", pos, ori
    )
    views.append(view)

    pos = config["furniture"]["round_table"]["round_table_base"]["reset_pos"][0]
    ori = config["furniture"]["round_table"]["round_table_base"]["reset_ori"][0][:3, :3]
    view = gen_prim(
        "round_table/round_table_base.usda", "/World/RoundTableBase", pos, ori
    )
    views.append(view)
