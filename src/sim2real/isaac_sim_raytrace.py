...
def main():
    ...
    # right_finger_view = XFormPrimView("/World/Robot_2/panda_leftfinger*", name="left_finger_xform_prim_view")
    # left_finger_view = XFormPrimView("/World/Robot_2/panda_rightfinger*", name="right_finger_xform_prim_view")
    ...
    for obs_idx, obs in enumerate(data['observations']):
        ...
        for i in range(int(sim_steps)):
            ...
            robot_world_poses = []
            for ii, body_name in enumerate(robot.articulations.body_names):
                if 'finger' in body_name:
                    continue
                body_position, rot_mat = kinematics_solver.compute_forward_kinematics(body_name, data['observations'][i]['robot_state']['joint_positions'])
                body_quat_wxyz = T.convert_quat(R.from_matrix(rot_mat).as_quat())
                body_pose_arr = np.concatenate([body_position, body_quat_wxyz])
                robot_world_poses.append(torch.from_numpy(body_pose_arr).float())
            link_world_poses = torch.stack(robot_world_poses[:-1])
            hand_world_pose = robot_world_poses[-1]

            # this function call below doesn't work (non-root links, already wrapped in the articulation view)
            # links_view.set_world_poses(positions=link_world_poses[:, :3], orientations=link_world_poses[:, 3:7])
