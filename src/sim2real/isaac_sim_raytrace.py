...
parser.add_argument('-i', '--demo-index',
                    type=int,
                    default=0)
parser.add_argument('-sub', '--sub-steps',
                    type=int,
                    default=0)
...
def main():
    ...
    output_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "output", "camera", f"{demo_date}_substeps_{args_cli.sub_steps}")
    rep_writer = rep.BasicWriter(output_dir=output_dir, frame_padding=3)

    wrist_output_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "output", "wrist_camera", f"{demo_date}_substeps_{args_cli.sub_steps}")
    ...
    # Lights-2
    # prim_utils.create_prim(
    #     "/World/Light/WhiteSphere2",
    #     "SphereLight",
    #     translation=(0.5, -0.8, 5.0),
    #     attributes={
    #         "radius": 4.5,
    #         "intensity": 1200.0,
    #         "color": (1.0, 1.0, 1.0)
    ...
    prim_utils.create_prim(
        '/World/ObstacleFront',
        translation=(0.3815, 0., table_z_offset),
    ...
    prim_utils.create_prim(
        ...
        orientation=obs_quat_wxyz,
        usd_path=str((usd_base_path / "obstacle_side.usda")))
    obstacle_center_view = XFormPrimView('/World/ObstacleFront', reset_xform_properties=False)
    obstacle_right_view = XFormPrimView('/World/ObstacleRight', reset_xform_properties=False)
    obstacle_left_view = XFormPrimView('/World/ObstacleLeft', reset_xform_properties=False)
    ...
    # obstacle_center_pose[2] += table_z_offset
    obstacle_right_pose = obstacle_center_pose.copy() # ; obstacle_right_pose[0] -= 0.075; obstacle_right_pose[1] -= 0.175
    obstacle_left_pose = obstacle_center_pose.copy() # ; obstacle_left_pose[0] -= 0.075; obstacle_left_pose[1] += 0.175
    ...
    for obs_idx, obs in enumerate(data['observations']):
        ...
        for i in range(int(sim_steps)):
            ...
            robot.set_dof_state(
                torch.concat([
                    interp_goal,
                    torch.from_numpy(fj1.reshape(1,)).float(),
                    torch.from_numpy(fj2.reshape(1,)).float()
                ]),
                torch.concat([
