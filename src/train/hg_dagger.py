...
def main(cfg: DictConfig):
    ...
    buffer = DaggerDataset(
        dataset_path=Path(
            "/tmp/scratch/furniture-data/processed/diffik/sim/one_leg/teleop/low/success.zarr"
        ),
    ...
    while iteration < cfg.num_iterations:
        ...
        if not eval_mode:
            ...
            episode_ends = torch.where(success_dones)[0] + 1
            episode_idxs = torch.where(success_dones)[1]

            # Only add the timesteps that are part of the episode
            for ep_idx, ep_len in zip(episode_idxs, episode_ends):
                buffer.add_trajectory(
                    {
                        "robot_state": success_robot_states[:ep_len, ep_idx]
                        .cpu()
                        .numpy(),
                        "action": success_actions[:ep_len, ep_idx].cpu().numpy(),
                        "color_image1": color_images1[:ep_len, ep_idx].cpu().numpy(),
                        "color_image2": color_images2[:ep_len, ep_idx].cpu().numpy(),
                    }
                )
        ...
        wandb.log(
            {
                ...
                "n_timesteps_in_buffer": len(buffer),
                "n_trajectories_in_buffer": len(buffer.episode_ends),
                "beta": beta_to_use,
