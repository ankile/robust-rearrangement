...
while true; do
    DATA_DIR_RAW=/data/scratch/ankile/robust-assembly-video-data python -m src.eval.evaluate_model \
        --n-envs 32 --n-rollouts 32 -f $task --if-exists append --max-rollout-steps $rollout_steps --controller diffik \
        --use-new-env --action-type pos --randomness $randomness --wt-type best_success_rate --run-id $run_id \
